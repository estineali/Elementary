...
void LinkedList<T2>::remove(int indx)
{
	...
	if (indx <= last_index)
	{
		...
		{
			Node<T2>* node_before_indx = m_first;
			for (int i = 0; i < indx-1; i++)
			{
				node_before_indx = node_before_indx->m_node_ahead;
			}
			Node<T2>* temp = node_before_indx->m_node_ahead;
			node_before_indx->m_node_ahead = node_before_indx->m_node_ahead->m_node_ahead;
			delete temp;
			m_size--;
		}
	}
