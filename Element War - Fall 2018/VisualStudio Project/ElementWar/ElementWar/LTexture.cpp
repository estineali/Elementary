...
bool LTexture::LoadFromFile(string path, SDL_Renderer* gRenderer, bool flagColorKey, 
							Uint8 redColorKey, Uint8 greenColorKey, Uint8 blueColorKey)
{
	...
	else
	{
		...
		{
			cout << "Unable to create texture from surface from " << path.c_str() << ". SDL_image error" << IMG_GetError() << endl;
		}
...
void LTexture::render(int x, int y, SDL_Renderer* rend, SDL_Rect* clip)
{
	...
	//Render to screen
	SDL_RenderCopy(rend, texture, clip, &renderQuad);
}
